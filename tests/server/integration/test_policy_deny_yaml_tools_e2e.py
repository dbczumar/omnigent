...
async def test_ungoverned_session_evaluate_marks_governed_false(
    client: httpx.AsyncClient,
) -> None:
    """A session with no policies at all gets ALLOW plus ``governed: false``.

    The flag is what lets the native-harness loopback relay cache the
    verdict briefly instead of paying a server round trip on every hook
    event (twice per tool call). It must appear only on the wholly
    ungoverned fast path: a governed session's verdict — even an ALLOW —
    must not carry ``governed: false``, or the relay would cache away a
    live policy set.
    """
    ...
    body = resp.json()
    assert body["result"] == "POLICY_ACTION_ALLOW"
    assert body.get("governed") is False, (
        f"ungoverned fast path must stamp governed=false; got {body}"
    )

    governed_agent = await create_test_agent(
        client,
        guardrails={
            "policies": {
                "deny_echo": {
    ...
    governed_sid = await _create_session(client, governed_agent["id"])
    resp = await client.post(
        f"/v1/sessions/{governed_sid}/policies/evaluate",
        json=_tool_call_request("grep"),
    )
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert body["result"] == "POLICY_ACTION_ALLOW"
    assert body.get("governed") is not False, (
        f"governed session must not be marked ungoverned: {body}"
    )
